...
using ExpKey   = Sugar::Key;
using ExpMove  = Sugar::Move;
using ExpValue = Sugar::Value;
using ExpDepth = Sugar::Depth;
...
struct ExpEntryEx: Current::ExpEntry {
    ...
    std::pair<int, bool> quality(Sugar::Position& pos, int evalImportance) const;
};
...
void show_exp(Sugar::Position& pos, bool extended);
